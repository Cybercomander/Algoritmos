...
int main(int argc, char ** argv){
    ...
    //PARA LA MODIFICACIÓN DE LA SEGUNDA MITAD DESCENDENTE:
    mediana = argc/2 ;

    //GENERACIÓN DE PILA PARA EL DESARROLLO DEL MERGE
    Pila pila;
    inicializar_pila(&pila); 
    push(&pila, mediana+1, argc-1, DIVIDIR); // Este push se usará para la segunda mitad descendente
    push(&pila, 0, mediana, DIVIDIR); // Este push se usará para la primer mitad ascendente
...
void merge(int arr[], int temp[], int izq, int med, int der){
    ...
    if (izq < mediana)
    {
        ...
        // Combina los elementos de ambas partes en orden descendente
        while(i <= med && j <= der)
            temp[k++] = (arr[i] >= arr[j]) ? arr[i++] : arr[j++];
        // Copia los elementos restantes de la parte izquierda
        while(i <= med)
